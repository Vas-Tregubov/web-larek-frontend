...
Класс Component является абстрактным и используется как базовый для всех компонентов слоя представления.

Конструктор класса:

- `container: HTMLElement` — Основной контейнер компонента, в который будет вставляться содержимое компонента.

Методы класса:

- `render(data?: Partial<T>): HTMLElement` — Метод для отображения данных в компоненте. Принимает объект данных типа `T` (или его частичную версию) и обновляет поля компонента, используя `Object.assign()`. Возвращает сам контейнер компонента (`container`), в который добавляется новое содержимое.
- `toggleElementState(element: HTMLElement, isActive: boolean): void` — Метод для управления состоянием элементов. Если `isActive` равно `false`, элемент будет отключен (через установку атрибута `disabled`), если `true` — элемент будет активирован (атрибут `disabled` будет удален).
- `createContent(): HTMLElement` — Абстрактный метод, который должен быть реализован в дочерних классах для создания и возврата содержимого компонента.
