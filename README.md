...
- `initialize(): void` — Метод для установки слушателей событий, таких как кнопка "В корзину" и закрытие модального окна по нажатию на кнопку "крестик" или клик в оверлей. При нажатии на кнопку "В корзину" вызывается сохраненный обработчик `handleAddToCart`.
...
- `open(cartItems: T[], onCheckout: (cartData: T[]) => void): void`  
  Расширяет метод `open` из базового класса `Modal`. Принимает массив товаров `T[]`, отображает их в корзине и устанавливает обработчик для оформления заказа.

- `close(): void`  
  Расширяет метод `close` из базового класса `Modal`. Скрывает модальное окно и сбрасывает состояние корзины.

- `initialize(): void`  
  Устанавливает слушатели событий для кнопок "Удалить товар" и "Оформить заказ". Также добавляет обработчики для подсчета и отображения общей суммы заказа.

- `removeItem(itemId: string): void`  
  Удаляет товар из корзины по идентификатору `itemId` и пересчитывает общую сумму заказа.

- `calculateTotal(): void`  
  Пересчитывает общую сумму заказа на основе товаров, добавленных в корзину.

- `updateCartItems(cartItems: T[]): void`  
  Обновляет отображаемые товары в корзине. Включает пересчет общей суммы.

- `submitOrder(): void`  
  Инициирует процесс оформления заказа, передавая массив товаров через событие или вызов обработчика `onCheckout`.
...
- `initialize(): void` — Устанавливает слушатели событий для кнопок и полей формы. Управляет переходами между этапами формы, валидацией данных, изменением текста на кнопках ("Далее", "Оплатить", "Оформить").
...
- `close(): void` — Расширяет метод `close` из базового класса `Modal`. Используется для скрытия модального окна и сброса состояния.
- `initialize(): void` — Устанавливает слушатели событий для кнопки "За новыми покупками", которая вызывает сохраненный обработчик `onReturnHome`.
