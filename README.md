...
- `open(cartItems: T[], onCheckout: (cartData: T[]) => void): void` — Расширяет метод `open` из базового класса `Modal`. Принимает массив товаров `T[]`, отображает их в корзине и устанавливает обработчик для оформления заказа.
- `close(): void` — Расширяет метод `close` из базового класса `Modal`. Скрывает модальное окно и сбрасывает состояние корзины.
...
- `calculateTotal(): void` — Пересчитывает общую сумму заказа на основе товаров, добавленных в корзину.
- `updateCartItems(cartItems: T[]): void` — Обновляет отображаемые товары в корзине. Включает пересчет общей суммы.
