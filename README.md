...
- `isValidAddress(address: string): boolean` — Проверяет, соответствует ли указанный телефон правильному формату российского номера. Возвращает `true`, если номер валиден, и `false` в противном случае.
...
Класс предназначен для работы с данными корзины, включая управление состоянием выбранных продуктов и расчет общей стоимости выбранных продуктов. Он также генерирует события, когда данные корзины обновляются.

Поля класса:

- `selectedProducts` — Массив продуктов, выбранных для корзины.

Методы класса:

- `getSelectedProducts()` — Возвращает список выбранных продуктов из корзины.
- `getTotalPrice()` — Рассчитывает общую стоимость выбранных продуктов в корзине.
- `resetSelected()` — Сбрасывает состояние выбора всех продуктов в корзине и генерирует событие `basket:changed`.
...
Класс `Page` представляет компонент для отображения каталога товаров на странице, а также управления корзиной покупок. Он позволяет обновлять счётчик товаров в корзине, отображать список товаров и блокировать страницу при необходимости.
...
- `gallery: HTMLElement` — Элемент, который представляет галерею (каталог товаров), в котором будут отображаться элементы.
- `basketButton: HTMLButtonElement` — Кнопка, которая открывает корзину при клике.
...
- `set counter(counter: number)` — Метод для обновления отображаемого счетчика товаров в корзине. Принимает число и отображает его в элементе с классом `.header__basket-counter`.
- `set catalog(items: HTMLElement[])` — Метод для обновления содержимого каталога. Принимает массив HTML-элементов и заменяет текущее содержимое каталога.
- `set locked(value: boolean)` — Метод для блокировки или разблокировки содержимого страницы. Если значение `value` равно `true`, добавляется класс `page__wrapper_locked`, который блокирует контент. Если `value` равно `false`, класс удаляется, разблокируя контент.

#### Класс Basket

Класс `Basket` представляет корзину покупок на странице и управляет отображением списка товаров, общей стоимостью и состоянием кнопки для оформления заказа. Он наследуется от базового класса `Component` и предоставляет методы для обновления контента корзины и управления состоянием элементов.
...
- `basketList: HTMLElement` — Элемент, содержащий список товаров корзины.
- `totalPrice: HTMLElement` — Элемент, отображающий общую стоимость корзины.
...
Представляет форму заказа, где пользователь может выбрать способ оплаты и указать адрес доставки. Класс наследуется от общего класса `Form` и реализует функционал обработки взаимодействий с кнопками и изменения ввода.
...
- `card` — Кнопка для выбора способа оплаты картой.
- `cash` — Кнопка для выбора способа оплаты наличными.
- `address` — Поле ввода для указания адреса доставки.

#### Класс CardBasket

Класс `CardBasket` представляет карточку товара в корзине. Он отображает информацию о товаре, такую как название, цена и индекс, а также управляет обработкой кликов по кнопке товара. Для удобства реализация всех классов, связанных с карточкой товара, сделана в одном файле `Card.ts`.

##### Конструктор класса:

- `constructor(container: HTMLElement, actions?: ICardActions)`  
  Инициализирует компонент, находит необходимые элементы в переданном контейнере и добавляет обработчик кликов для кнопки товара, если передан `actions.onClick`.

##### Поля класса:

- `itemIndex` — Элемент, отображающий индекс товара в корзине.
- `cardButton` — Кнопка для взаимодействия с товаром.
- `cardTitle` — Элемент, содержащий название товара.
- `cardPrice` — Элемент, отображающий цену товара.
...
- `handlePrice(value: number | null)` — Форматирует цену товара. Возвращает строку с ценой и единицей измерения или текст "Бесценно", если цена равна `null`.

- `set price(price: number)` — Устанавливает цену товара. Если цена равна `0`, кнопка товара становится недоступной.

- `set index(value: number)` — Устанавливает индекс товара в корзине.

#### Класс CardList

Класс представляет карточку товара в каталоге, отображая категорию, изображение и управляя состоянием выбора товара.

Конструктор класса:

- `constructor(container: HTMLElement, actions?: ICardActions)` — Инициализирует компонент, находит необходимые элементы на странице и добавляет обработчик события для кнопки, если передан объект `actions`.
...
- `set category(category: CategoryType)` — Устанавливает категорию товара, обновляет текст элемента и добавляет соответствующий CSS класс.
- `set image(image: string)` — Устанавливает URL источника изображения товара.
- `set selected(value: boolean)` — Управляет состоянием кнопки товара, отключая её при установке `value` в `true`.

#### Класс CardPreview

Класс представляет карточку товара с превью, расширяющую функциональность
карточки из `CardList`, добавляя возможность отображать описание товара.

Конструктор класса:

- `constructor(container: HTMLElement, actions?: ICardActions)` — Инициализирует компонент, находит необходимые элементы на странице и устанавливает обработчик события для кнопки, если передан `actions.onClick`.
